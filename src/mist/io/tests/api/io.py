...
class MistIoApi(object):
    ...
    def add_cloud(self, title, provider, apikey, apisecret, region, apiurl=None,
                    tenant_name=None, csrf_token=None, Api_Version=None):
        ...
        req = MistRequests(uri=self.uri + '/clouds', data=json.dumps(payload),
                           csrf_token=csrf_token, Api_Version='2')
    ...
    def cloud_action(self, cloud_id,action_type, title='', provider='', apikey='', apisecret='',
                     region='', dry='',new_state='', apiurl=None,
    ...
    def delete_cloud(self, cloud_id, csrf_token=None):

        req = MistRequests(uri=self.uri+'/clouds/'+cloud_id, csrf_token=csrf_token)
